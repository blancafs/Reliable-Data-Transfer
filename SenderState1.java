...
public class SenderState1 extends SenderState{
	...
	public void sendPacket(Sender1b sender, DatagramPacket packet, DatagramSocket socket) throws Exception {
		...
		while (sending) {
			System.out.println("Sending packet seq=1 ....");
//			// Receiving old ack ???
...
//			System.out.println("Received packet from " + packet.getSocketAddress());
			...
			// This means correct acknowledgement sent, we have sent the right packet!
			if (seqs2.contains("1")) {
				sending = false;
				sender.changeState(new SenderState0());
				System.out.println("Correct packet sent! Moved on to SenderState0.");
			} else if (!(seqs2.contains("1"))) {
				System.out.println("Wrong ack=0 received, resending packet.");
